...
export const ListPage: React.FC = () => {
  ...
  const addToTail = async () => {
    ...
    setDisabled({...disabled, addHead: true,
      deleteHead: true,
      deleteTail: true,
      addIndex: true,
      deleteIndex: true})
    await setTimeout(() => {
      ...
      setDisabled({...disabled, addHead: false,
        deleteHead: false,
        deleteTail: false,
        addIndex: false,
        deleteIndex: false})
    ...
    <SolutionLayout title="Связный список">
      <div className={styles.container}>
        <form className={styles.enter_value}>
          <Input disabled={inputValueDisabled} isLimitText type="text" maxLength={4} extraClass={styles.input} placeholder="Введите значение" value={value} onChange={(e) => {
            e.preventDefault();
        ...
        <form className={styles.enter_index}>
          ...
          <div className={styles.buttons_index}>
            <Button extraClass={styles.button_index} isLoader={buttonsLoader.addIndex} disabled={value === '' || inputIndex === '' ? true : disabled.addIndex} text="Добавить по индексу" onClick={() => {
              addByIndex()
            }} />
            <Button extraClass={styles.button_index} isLoader={buttonsLoader.deleteIndex} disabled={inputIndex === '' ? true : disabled.deleteIndex} text="Удалить по индексу" onClick={() => {
              deleteByIndex()
            }} />
          </div>
        </form>
